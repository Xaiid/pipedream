...
export default {
  ...
  async run({ $ }) {
    const { infobip, username, password, bulkId, from, to, text, flash, transliteration, languageCode, intermediateReport, notifyUrl, notifyContentType, callbackData, validityPeriod, sendAt, track, processKey, trackingType, indiaDltContentTemplateId, indiaDltPrincipalEntityId, indiaDltTelemarketerId, turkeyIysBrandCode, turkeyIysRecipientType, southKoreaResellerCode, ...params } = this;

    const pathQuery = [];
    if (username !== undefined && username !== null) pathQuery.push({ name: "username", value: username.toString() });
    if (password !== undefined && password !== null) pathQuery.push({ name: "password", value: password.toString() });
    ...
    if (track !== undefined && track !== null) pathQuery.push({ name: "track", value: track.toString() });
    if (processKey !== undefined && processKey !== null) pathQuery.push({ name: "processKey", value: processKey.toString() });
